...
class Rocket:
    ...
    def f2(self, v, i:int, r0, distanceToSun, relativeVelocity):
        ...
        if self.planetNearEnough:
            ...
            x = ( -(G*self.nearestPlanet.mass/r0**2) - (Luftwiederstand*relativeVelocity**2*np.sign(self.v_x[i]) * p_0 * np.exp(-abs((r0-self.nearestPlanet.radius)) / h_s))/(2 * self.KoerperMasse) ) * ((self.r_x[i] - self.nearestPlanet.r_x[i])/r0) #Extrakraft x einbauen
    ...
    def f1(self, v,i:int, r0, distanceToSun, relativeVelocity):
        ...
        if self.planetNearEnough:
            #r0 = np.sqrt( (self.r_x[i] - self.nearestPlanet.r_x[i])**2 + (self.r_z[i] - self.nearestPlanet.r_z[i])**2)
            z = ( -(G*self.nearestPlanet.mass/r0**2) - (Luftwiederstand*relativeVelocity**2*np.sign(self.v_z[i]) * p_0 * np.exp(-abs((r0-self.nearestPlanet.radius)) / h_s))/(2 * self.KoerperMasse) ) * ((self.r_z[i] - self.nearestPlanet.r_z[i])/r0)
    ...
    def berechneNaechstenSchritt(self, i: int, planets):
        ...
        if self.planetNearEnough:
           r0 = np.sqrt( (self.r_x[i] - self.nearestPlanet.r_x[i])**2 + (self.r_z[i] - self.nearestPlanet.r_z[i])**2)
           relativeVelocity = self.getRelativeVelocity(i)
        distanceToSun = np.sqrt( (self.r_x[i] - self.sun.r_x[i])**2 + (self.r_z[i] - self.sun.r_z[i])**2)
