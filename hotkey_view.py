...
def changeHotKeyFromInput(event,hotkey,object_id):
    if event.type == pygame_gui.UI_TEXT_ENTRY_FINISHED and event.ui_object_id == object_id:
        hotkey = ord(event.text)
    return hotkey
    ...
    while showGUI:
        for event in pygame.event.get():
            if checkKeyDown(event, H_closeWindow[0]):
                showGUI = False
            H_centerOnRocket =changeHotKeyFromInput(event, H_centerOnRocket, 1)
            if event.type == pygame_gui.UI_TEXT_ENTRY_FINISHED and event.ui_object_id == 1:
